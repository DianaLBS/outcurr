...
public class AcadProgramServiceImpl implements AcadProgramService {
    ...
    // TODO the program should not assume the operations are for CURRENT programs.
    // It should also support Future
    // and Inactive, filterin according to the parameter used as input. Use the
    // logger for errors.
    ...
    public AcadProgramOutDTO getAcadProgram(long facultyId, long acadProgramId) {
        ...
        return meterRegistry.timer("acadProgram.getById").record(() -> {
            try {
                ...
                logger.info("Successfully retrieved academic program | facultyId={}, acadProgramId={}", facultyId,
                        acadProgramId);
                return programOutDTO;

            } catch (Exception e) {
                logger.error("Error in getAcadProgram | facultyId={}, acadProgramId={}, message={}", facultyId,
                        acadProgramId, e.getMessage(), e);
                throw e;
    ...
    // TODO enable AspectJ for non-injected cache calls, change visibility to
    // non-public
    ...
    public AcadProgramOutDTO createAcadProgram(long facultyId, AcadProgramInDTO acadProgramInDTO) {
        ...
        try {
            ...
            logger.debug("Mapping AcadProgramInDTO to AcadProgram | facultyId={}, acadProgramInDTO={}", facultyId,
                    acadProgramInDTO);
            ...
            AcadProgramOutDTO createdProgram = acadProgramMapper
                    .acadProgramToAcadProgramOutDto(acadProgramRepository.save(acadProgram));

            // Incrementar contador de métrica
            meterRegistry.counter("acadProgram.created").increment();

            logger.info("Successfully created academic program | facultyId={}, acadProgramOutDTO={}", facultyId,
                    createdProgram);
            return createdProgram;

        } catch (Exception e) {
            logger.error("Error in createAcadProgram | facultyId={}, acadProgramInDTO={}, message={}", facultyId,
                    acadProgramInDTO, e.getMessage(), e);
            throw e;
    ...
    public void updateAcadProgram(long facultyId, long acadProgramId, AcadProgramInDTO acadProgramInDTO) {
        ...
        // TODO: validate acadProgram is in faculty. Throw exception if program does not
        // exists
